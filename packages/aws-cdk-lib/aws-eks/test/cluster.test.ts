...
let proxyAgentcodePath = '';

afterEach(() => {
  try {
    if (!proxyAgentcodePath) {
      return;
    }
    log('Removing directory:', proxyAgentcodePath);
    // fs.rmSync(proxyAgentcodePath, { recursive: true });
  } catch (error) {
...
describe('cluster', () => {
  test('can configure and access ALB controller', () => {
    ...
    proxyAgentcodePath = cluster.nodeProxyAgentCodePath;
